...
func startTraining(charName, charClassclass string) string {
	if charClassclass == "warrior" {
		fmt.Printf("%s, ты Воитель - отличный боец ближнего боя.\n", charName)
	}

	if charClassclass == "mage" {
		fmt.Printf("%s, ты Маг - превосходный укротитель стихий.\n", charName)
	}

	if charClassclass == "healer" {
	...
	for cmd != "skip" {
		...
		if cmd == "attack" {
			fmt.Println(attack(charName, charClassclass))
		}

		if cmd == "defence" {
			fmt.Println(defence(charName, charClassclass))
		}

		if cmd == "special" {
			fmt.Println(special(charName, charClassclass))
		}
	}
...
func main() {
	...
	char := choiceCharClass()

	fmt.Println(startTraining(charName, char))
}
